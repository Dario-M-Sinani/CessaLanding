  ...
  <AppLayout>
    <div class="py-16 bg-white min-h-screen">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div class="space-y-6">
          <div v-if="outages.data && outages.data.length" class="space-y-5">
            <div v-for="outage in outages.data" :key="outage.id" class="bg-gray-50 border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
              <div class="flex flex-col sm:flex-row sm:items-center gap-3 bg-blue-950 text-white px-5 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-base sm:text-lg font-extrabold">{{ formatFechaLarga(outage.execution_date) }}</span>
                </div>
                <div class="flex items-center gap-2 sm:ml-auto bg-amber-400 text-blue-950 px-4 py-1.5 rounded-full font-mono font-extrabold text-base sm:text-lg whitespace-nowrap">
              ...
              <div class="p-5 space-y-3">
                <h3 class="text-lg font-bold text-blue-950">Motivo: {{ outage.reason }}</h3>
          ...
          <div v-else class="p-8 bg-gray-50 border border-gray-200 rounded-2xl text-center space-y-3 shadow-sm">
            ...
            <h3 class="text-xl font-bold text-blue-950">No hay cortes programados a la fecha</h3>
            <p class="text-gray-600 text-sm">La red eléctrica de CESSA opera con normalidad sin trabajos de mantenimiento previstos.</p>
          </div>

          <div v-if="outages.links && outages.links.length > 3" class="flex flex-wrap items-center justify-center gap-2 pt-4">
            <Link
              v-for="(link, idx) in outages.links"
              :key="idx"
              :href="link.url || '#'"
              v-html="link.label"
              :class="[
