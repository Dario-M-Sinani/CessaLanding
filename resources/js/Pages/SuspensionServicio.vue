        ...
        <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6 sm:p-10 shadow-sm space-y-8">
          ...
          <form v-else @submit.prevent="submitForm" class="space-y-8">
            ...
            <div v-if="currentStep === 1" class="space-y-4">
              ...
              <p class="text-xs text-gray-500">Para tramitar la suspensión, tu cuenta no debe tener deuda pendiente hasta la última lectura de tu medidor.</p>
            ...
            <div v-else-if="currentStep === 2" class="space-y-6">
              ...
              <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <label v-for="opt in serviceTypeOptions" :key="opt.value" class="flex items-center gap-2 px-4 py-3 bg-white border rounded-xl text-sm cursor-pointer transition-colors" :class="form.service_type === opt.value ? 'border-blue-900 ring-1 ring-blue-900' : 'border-gray-300'">
                    <input type="radio" :value="opt.value" v-model="form.service_type" class="accent-blue-900" />
              ...
              <div class="space-y-4 pt-2 border-t border-gray-200">
                <h4 class="text-sm font-bold text-blue-950 pt-4">Última Lectura del Medidor</h4>
                <PhotoUploadField label="Fotografía del medidor" variant="meter" v-model="form.url_last_meter_reading" />
                <div>
                  <label class="block text-xs font-medium text-gray-700 mb-1">Valor Leído en el Medidor *</label>
                  <input
                    :value="form.last_meter_reading"
                    @input="onDigitsOnlyInput($event, 'last_meter_reading', 12)"
                    type="text"
                    inputmode="numeric"
                    pattern="[0-9]*"
                    maxlength="12"
                    placeholder="Solo números, máx. 12 dígitos"
                    class="w-full sm:w-72 px-4 py-3 bg-white border rounded-xl text-gray-900 text-sm focus:border-blue-900 focus:outline-none"
                    :class="touched.step2 && !form.last_meter_reading ? 'border-red-400' : 'border-gray-300'"
                  />
                </div>
                <p v-if="touched.step2 && (!form.url_last_meter_reading || !form.last_meter_reading)" class="text-xs font-semibold text-red-600">Debes subir la foto del medidor y registrar el valor leído.</p>
              </div>
            ...
            <div v-else class="space-y-4">
              ...
              <div class="pt-2 flex gap-3">
                ...
                >
                  <span v-if="!submitting">Enviar Solicitud de Suspensión</span>
...
const props = defineProps({
  pageBadge: { type: String, default: 'Suspensión de Servicio' },
  pageTitle: { type: String, default: 'Suspensión Temporal o Definitiva' },
  pageDescription: {
    type: String,
    default: 'Solicita la suspensión temporal o definitiva del suministro eléctrico de tu inmueble.',
  },
...
const serviceTypeOptions = [
  ...
  { value: 'SUSPENSION_DEFINITIVA', label: 'Suspensión Definitiva' },
  { value: 'SUSPENSION_INSPECCION', label: 'Suspensión con Inspección' },
];

const form = reactive({
  fullname: '',
  email: '',
  service_type: 'SUSPENSION_TEMPORAL',
  document_number: '',
  url_document_front: null,
  url_document_back: null,
  url_invoice: null,
  url_last_meter_reading: null,
  last_meter_reading: '',
  phone: '',
  mobile_phone: '',
  address: '',
...
const STEP_FIELDS = {
  1: ['fullname', 'email'],
  2: ['service_type', 'document_number', 'url_document_front', 'url_document_back', 'url_invoice', 'url_last_meter_reading', 'last_meter_reading'],
...
const step2Valid = computed(() => !!form.service_type
  && !!form.document_number
  && !!form.url_document_front
  && !!form.url_document_back
  && !!form.url_invoice
  && !!form.url_last_meter_reading
  && !!form.last_meter_reading);
