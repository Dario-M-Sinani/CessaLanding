...
class ActualizarDatosController extends Controller
{
    ...
    public function confirmarCodigo(Request $request): JsonResponse
    {
        $cuenta = $this->cuentaVerificada($request);
        ...
        if (! $otp) {
            return response()->json([
                'success' => false,
                'message' => 'El código venció. Solicita un nuevo envío.',
            ], 419);
        }
        ...
        if ($otp['intentos'] >= self::OTP_MAX_INTENTOS) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Demasiados intentos fallidos. Solicita un nuevo envío del código.',
            ], 429);
        }

        if (! Hash::check($validated['codigo_email'], $otp['email_code_hash'])) {
            $otp['intentos']++;
