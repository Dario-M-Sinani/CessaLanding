      ...
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div class="text-center space-y-3">
          ...
          <p class="text-gray-600 text-sm max-w-2xl mx-auto">
            ...
            directo a tu <span class="font-semibold text-gray-800">correo electrónico</span>.
            Confirmar tu correo es el primer paso para que no se te pase ninguna factura.
          </p>
        </div>
        ...
        <div class="flex items-center justify-center gap-2 sm:gap-4">
          <div
            v-for="(label, idx) in ['Verificar Cuenta', 'Correo Electrónico', 'Confirmar Código']"
            :key="idx"
        ...
        <div v-if="paso === 2" class="bg-gray-50 border border-gray-200 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm">
          ...
          <form @submit.prevent="enviarCodigo" class="space-y-4">
            <div>
        ...
        <div v-if="paso === 3" class="bg-gray-50 border border-gray-200 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm">
          <div class="pb-4 border-b border-gray-200">
            ...
            <h2 class="text-xl font-black text-gray-900 mt-1">Confirma el código que enviamos</h2>
            <p class="text-xs text-gray-600 mt-1">
              Enviamos un código a <span class="font-semibold text-gray-800">{{ contacto.email }}</span>. Vence en 10 minutos.
            </p>
          </div>

          <form @submit.prevent="confirmarCodigo" class="space-y-4">
            <div class="max-w-xs">
...
const enviarCodigo = async () => {
  ...
  try {
    ...
    if (json.success) {
      ...
      error.value = json.message || 'No se pudo enviar el código.';
    }
  } catch (e) {
    error.value = 'No se pudo conectar con el servidor. Intenta más tarde.';
  } finally {
...
const confirmarCodigo = async () => {
  ...
  try {
    const json = await postJson('/api/actualizar-datos/confirmar-codigo', {
    ...
    if (json.success) {
      ...
      error.value = json.message || 'El código no es válido.';
    }
  } catch (e) {
    error.value = 'No se pudo conectar con el servidor. Intenta más tarde.';
  } finally {
