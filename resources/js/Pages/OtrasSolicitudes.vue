        ...
        <div class="bg-gray-50 border border-gray-200 rounded-2xl p-6 sm:p-10 shadow-sm space-y-8">
          ...
          <form v-else @submit.prevent="submitForm" class="space-y-8">
            ...
            <div v-if="currentStep === 1" class="space-y-4">
              <div class="space-y-2">
                <p class="text-xs text-gray-600">Estimado cliente, le informamos que con este formulario podrá realizar las siguientes solicitudes:</p>
                <ul class="list-disc list-inside text-xs text-gray-600 space-y-0.5">
                  <li>Traslado en la misma casa</li>
                  <li>Cambio de Nombre</li>
                  <li>Cambio de datos del cliente</li>
                  <li>Rehabilitación del suministro</li>
                  <li>Rehabilitación con inspección</li>
                  <li>Cambio o asignación de NIT</li>
                </ul>
                <p class="text-xs font-bold text-red-600">NOTA: Todas estas solicitudes deben ser realizadas por el titular de la cuenta y no así por el poseedor.</p>
              </div>

              <h3 class="text-base font-bold text-blue-950 pt-2 border-t border-gray-200">Paso 1: Datos Personales</h3>

              <div class="space-y-2">
                <p class="text-xs text-gray-600">
                  Cliente <b>POSEEDOR</b> es la denominación utilizada para identificar a la persona que tiene asignada la
                  responsabilidad de efectuar el pago por consumo de energía eléctrica en calidad de inquilino, arrendador o anticresista.
                </p>
                <label class="block text-xs font-medium text-gray-700 mb-1">Tipo de Cliente *</label>
                <div class="grid grid-cols-2 gap-3">
                  <label class="flex items-center gap-2 px-4 py-3 bg-white border rounded-xl text-sm cursor-pointer transition-colors" :class="form.user_type === 'POSSESSOR' ? 'border-blue-900 ring-1 ring-blue-900' : 'border-gray-300'">
                    <input type="radio" value="POSSESSOR" v-model="form.user_type" class="accent-blue-900" />
                    <span class="font-medium text-gray-800">Poseedor</span>
                  </label>
                  <label class="flex items-center gap-2 px-4 py-3 bg-white border rounded-xl text-sm cursor-pointer transition-colors" :class="form.user_type === 'OWNER' ? 'border-blue-900 ring-1 ring-blue-900' : 'border-gray-300'">
                    <input type="radio" value="OWNER" v-model="form.user_type" class="accent-blue-900" />
                    <span class="font-medium text-gray-800">Titular</span>
                  </label>
                </div>
              </div>
            ...
            <div v-else-if="currentStep === 2" class="space-y-6">
              <h3 class="text-base font-bold text-blue-950">Paso 2: Datos de la Solicitud</h3>
              <p class="text-xs font-bold text-red-600">NOTA: Para los servicios de cambio en el registro del servicio la solicitud debe ser realizada por el Titular de la cuenta.</p>
              ...
              <div>
                ...
                <select
                  ...
                  class="w-full px-4 py-3 bg-white border rounded-xl text-gray-900 text-sm focus:border-blue-900 focus:outline-none"
                  :class="touched.step2 && !form.request_type_id ? 'border-red-400' : 'border-gray-300'"
                >
                  <option value="">-- Seleccione --</option>
                  <option v-for="(label, id) in requestTypeOptions" :key="id" :value="id">{{ label }}</option>
                </select>
                <p v-if="touched.step2 && !form.request_type_id" class="mt-1 text-xs font-semibold text-red-600">Este campo es obligatorio.</p>
              </div>
              ...
              <div class="space-y-4 pt-2 border-t border-gray-200">
                ...
                <div>
                  ...
                  <input
                    ...
                    @input="onDigitsOnlyInput($event, 'document_number', 15)"
                    type="text"
                    inputmode="numeric"
                    pattern="[0-9]*"
                    maxlength="15"
              ...
              <div class="space-y-4 pt-2 border-t border-gray-200">
                <h4 class="text-sm font-bold text-blue-950 pt-4">Última Factura Cancelada</h4>
                <PhotoUploadField label="Fotografía de la última factura cancelada" variant="invoice" v-model="form.url_invoice" />
                <p v-if="touched.step2 && !form.url_invoice" class="text-xs font-semibold text-red-600">Debes subir la fotografía de tu última factura cancelada.</p>
              </div>
...
const props = defineProps({
  pageBadge: { type: String, default: 'Otros Trámites' },
  pageTitle: { type: String, default: 'Otras Solicitudes' },
  pageDescription: {
    type: String,
    default: 'Registra cualquier otro trámite o solicitud relacionada con tu servicio eléctrico.',
  },
  googleMapsApiKey: { type: String, default: '' },
  requestTypeOptions: { type: Object, default: () => ({}) },
});
...
const form = reactive({
  ...
  latitude: null,
  longitude: null,
  service_type: 'OTRAS_SOLICITUDES',
});

const STEP_FIELDS = {
  1: ['user_type', 'fullname', 'email'],
  2: ['request_type_id', 'area', 'consumer_type', 'document_number', 'url_document_front', 'url_document_back', 'url_invoice'],
...
const step1Valid = computed(() => !!form.user_type && !!form.fullname && isEmailValid.value);
const step2Valid = computed(() => !!form.request_type_id
  && !!form.area
  && !!form.consumer_type
  && !!form.document_number
  && !!form.url_document_front
  && !!form.url_document_back
  && !!form.url_invoice);

const onDigitsOnlyInput = (e, field, maxLength) => {
  form[field] = e.target.value.replace(/\D/g, '').slice(0, maxLength);
};
